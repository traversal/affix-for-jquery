...
(function($) {
	...
	$.affixPosition = function(from, to, options)
	{
	  ...
	  var tp, ts, fs, fp, wo, ws, settings, info;
  	...
  	var WIN = {
  		...
  		{
  		  var settings = $(from).data("affix-settings");
    ...
    // affixPosition function body starts here!
		...
		var f = $(from).first();
    $(f).data("affix-settings", $.affixPosition.getSettings(options));
    
    var settings = $(f).data("affix-settings");
  ...
  $.fn.affixReset = function() {
    ...
    $(this).each( function() {
      ...
      from.data("affix-to", to);
    	from.data("affix-options", options);

      var fReaffix = function(event) {
        if (from && from.is(":visible")) { 
          el.reaffix(); 
        }
      };
      
      if (settings.autoEvents.resize && !from.data("affix-window-on-resize")) {
    	  from.data("affix-window-on-resize", fReaffix);
        $(window).resize(from.data("affix-window-on-resize"));
      }

      if (settings.autoEvents.resize && !from.data("affix-window-on-scroll")) {
    	  from.data("affix-window-on-scroll", fReaffix);
        $(window).scroll(from.data("affix-window-on-scroll"));
      }
              
			if (settings.shim) {
			    from.shim( { left: info.left, top: info.top } );
		    }
